...
impl Pool {
    ...
    pub fn withdraw_raw(&self, user: &User, withdraw_amount: u128) -> CallResult {
        desoroban_result(self.client.try_withdraw(&user.as_address(), &withdraw_amount))
    }
    ...
    pub fn deposit_by_id(&self, user: &Address, deposit_amount: f64) -> CallResult {
        desoroban_result(self.client.try_deposit(user, &self.float_to_int(deposit_amount)))
    }
