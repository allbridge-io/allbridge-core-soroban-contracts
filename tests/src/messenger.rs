...
use crate::utils::{
    ...
    BridgeEnv, Messenger, MessengerConfig, GAS_AMOUNT,
};
...
fn messenger_init() {
    ...
    let gas_oracle_id = Address::random(&env);
...
pub fn messenger_receive_message_full() {
    ...
    let yaro_token = Address::random(&env);
    let goerli_bridge = Address::random(&env);
    ...
    let message_hash = hash_message(
        ...
        GOERLI_CHAIN_ID,
        CHAIN_ID,
        &yaro_token.contract_id(),
        &U256::from_u32(&env, 8247),
    );
    let message_hash_with_sender =
        hash_with_sender(&env, &message_hash, &goerli_bridge.contract_id());
